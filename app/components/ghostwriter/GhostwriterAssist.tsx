  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        onClick={handleOpen}
        className={cn(
          'inline-flex w-fit items-center gap-2 rounded-full border border-dashed border-action-primary/40 bg-action-primary/5 px-3 py-1.5 font-sans text-xs font-semibold text-action-primary',
          'transition-colors hover:bg-action-primary/10',
          'focus-visible:outline-none focus-visible:[box-shadow:var(--focus-ring)]',
        )}
      >
        <FontAwesomeIcon icon={faPenFancy} aria-hidden className="h-3 w-3" />
        {t('button')}
      </button>

      {open && (
        <div className="flex flex-col gap-3 rounded-2xl border border-border-subtle bg-action-primary/5 p-4">
          ...
          {!suggestions && !busy && !error && (
            <div className="flex flex-col gap-2 rounded-xl border border-dashed border-border-default bg-surface-card p-4 text-sm text-text-muted">
              <p>{t('uploadPrompt')}</p>
              <input
                ref={fileRef}
                type="file"
          ...
          {suggestions && (
            <div className="flex flex-col gap-3">
              ...
              <button
                type="button"
                onClick={() => fileRef.current?.click()}
                className="inline-flex w-fit items-center gap-2 self-end rounded-full border border-border-subtle bg-surface-card px-3 py-1 text-xs font-medium text-text-muted hover:bg-surface-subtle"
              >
                {t('reanalyze')}
              </button>
              <input
