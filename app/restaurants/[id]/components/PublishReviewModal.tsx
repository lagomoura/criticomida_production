...
const PRICE_TIERS: { value: PriceTier; symbol: string; label: string }[] = [
  { value: '$', symbol: '$', label: 'Económico' },
  { value: '$$', symbol: '$$', label: 'Moderado' },
  { value: '$$$', symbol: '$$$', label: 'Premium' },
];
...
export default function PublishReviewModal({
  ...
  async function resolveDishId(): Promise<string> {
    ...
    if (selection?.kind !== 'new') {
      throw new Error('Falta seleccionar un plato.');
    }
    ...
    >
      ...
      >
        ...
        <header className="relative px-6 pt-5 pb-4 sm:px-8 sm:pt-7 sm:pb-5">
          ...
          >
            {selection ? `Reseñar “${dishName}”` : '¿Qué probaste?'}
          </h2>
          <p className="mt-1.5 font-sans text-sm leading-snug text-text-muted">
            {selection
              ? 'Compartí los detalles que importan: textura, punto, porción.'
              : 'Buscá en el menú o creá un plato nuevo.'}
          </p>
        ...
        <div className="overflow-y-auto px-6 py-5 sm:px-8 sm:py-6">
          ...
          {selection && (
            <div className="flex flex-col gap-5">
              ...
              {/* Review form */}
              <SectionLabel>Tu reseña</SectionLabel>
              <DishReviewForm
    ...
    <div className="flex flex-col gap-4">
      <div className="relative">
        ...
        <input
          ...
          placeholder={isMenuEmpty ? 'Tipeá el nombre del plato…' : 'Buscá en el menú o tipeá uno nuevo…'}
          value={query}
      ...
      {filtered.length > 0 && (
        <div className="flex flex-col gap-1.5">
          <SectionLabel>Del menú</SectionLabel>
          <ul className="flex flex-col gap-1.5" aria-label="Platos del menú">
            {filtered.map((dish) => (
              <li key={dish.id}>
                <button
                  type="button"
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      ...
                      {dish.review_count > 0 && (
                        <span className="font-sans text-xs text-text-muted">
                          {dish.review_count} reseña{dish.review_count !== 1 ? 's' : ''}
                        </span>
                      )}
                    </div>
                  </div>
      ...
      {showCreateOption && (
        <div className="flex flex-col gap-1.5">
          <SectionLabel>Nuevo en el menú</SectionLabel>
          <button
            type="button"
            onClick={onPickNew}
            className={[
      ...
      {filtered.length === 0 && !showCreateOption && (
        <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-border-default bg-surface-page px-6 py-10 text-center">
          ...
          <p className="m-0 font-display text-lg font-medium text-text-primary">
            {isMenuEmpty ? 'Aún no hay platos' : 'Sin coincidencias'}
          </p>
          <p className="m-0 max-w-xs font-sans text-sm text-text-muted">
            {isMenuEmpty
              ? 'Sé el primero. Tipeá un nombre y empezá tu reseña.'
              : 'Probá con otro nombre o tipeá uno nuevo para crearlo.'}
          </p>
        </div>
      )}
    </div>
...
function NewDishInlineFields({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-border-subtle bg-surface-page p-4">
      <SectionLabel>Detalles del nuevo plato · opcional</SectionLabel>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Textura, ingredientes, punto, porción"
          value={description}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-[1fr,auto]">
        <div>
          ...
          <div role="radiogroup" aria-label="Rango de precio" className="grid grid-cols-3 gap-2">
            {PRICE_TIERS.map((opt) => {
