...
function formatPriceLevel(level: number, unspecified: string): string {
  if (level === 0) return unspecified;
  return '$'.repeat(level);
}
...
function PlacesSearchInput({
  ...
  const t = useTranslations('addRestaurantModal');
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (restaurant.existed) {
        toast.info(
          t('existedToastTitle'),
          t('existedToastBody', { name: restaurant.name }),
        );
        onClose();
    ...
    >
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-neutral-200 px-6 py-4">
          ...
          <button type="button" className="cc-btn-close" aria-label={t('close')} onClick={onClose}>
            ×
          </button>
        </div>
            ...
            <div className="flex flex-col gap-4 px-6 py-5">
              ...
              {hasImportedDetails && (
                <div className="rounded-lg bg-neutral-50 px-4 py-3 text-sm">
                  <p className="mb-2 font-medium text-neutral-800">{t('placeDataHeading')}</p>
                  ...
                  {priceLevel !== undefined && (
                    <div className="mt-1 flex items-start gap-2 text-neutral-700">
                      <span aria-hidden="true">💰</span>
                      <span>{formatPriceLevel(priceLevel, t('priceLevelUnspecified'))}</span>
                    </div>
                  )}
            ...
            <div className="flex flex-col gap-4 px-6 py-5">
              ...
              <div>
                <label htmlFor="restaurant-category" className="form-label">
                  {t('categoryLabel')} {t('categoryRequired')}
                </label>
                ...
                {!newCategoryMode ? (
                  <>
                    ...
                    >
                      {categories.length === 0 && (
                        <option value="" disabled>{t('categoryLoading')}</option>
                      )}
                      ...
                      <div className="mt-2 rounded-lg border border-neutral-200 bg-neutral-50 p-3 text-sm">
                        {duplicateCategory ? (
                          <div className="text-amber-700">
                            <p className="font-medium">{t('duplicateCategoryWarning', { name: duplicateCategory.name })}</p>
                            <button
                              type="button"
                          ...
                          <>
                            <p className="font-medium text-neutral-800">
                              {t('newCategoryPreview', { name: newCategoryName.trim() })}
                            </p>
                            ...
                            <p className="mt-1.5 text-xs text-emerald-600">{t('noDuplicateCategory')}</p>
                          </>
                        )}
                      </div>
                    )}
              ...
              <div>
                <label htmlFor="restaurant-name" className="form-label">
                  {t('nameLabel')} {t('categoryRequired')}
                </label>
                <input
                  id="restaurant-name"
                  type="text"
              ...
              <div>
                <label htmlFor="restaurant-location" className="form-label">
                  {t('locationLabel')} {t('categoryRequired')}
                </label>
                <input
    ...
    <>
      <div className="px-6 py-5">
        <p className="mb-1 text-sm font-medium text-neutral-900">
          {isPlural ? t('matchPanelTitlePlural') : t('matchPanelTitleSingular')}
        </p>
        <p className="mb-4 text-sm text-neutral-600">
          {isPlural ? t('matchPanelBodyPlural') : t('matchPanelBodySingular')}
        </p>
