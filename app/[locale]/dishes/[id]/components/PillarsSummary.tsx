...
export default function PillarsSummary({ pillars }: PillarsSummaryProps) {
  const t = useTranslations('dish.pillarsSummary');
  const totalAnswered =
  ...
  const PILLARS: PillarMeta[] = useMemo(() => [
    {
      ...
      options: [
        { value: 1, emoji: '📸', label: t('presentationPoor'), tone: 'negative' },
        { value: 2, emoji: '🆗', label: t('presentationNormal'), tone: 'neutral' },
        { value: 3, emoji: '🌟', label: t('presentationGreat'), tone: 'positive' },
      ],
    },
    {
      key: 'valueProp',
      ...
      hint: t('valueHint'),
      options: [
        { value: 1, emoji: '💸', label: t('valueExpensive'), tone: 'negative' },
        { value: 2, emoji: '⚖️', label: t('valueFair'), tone: 'neutral' },
        { value: 3, emoji: '💎', label: t('valueGreat'), tone: 'positive' },
      ],
    },
    {
      key: 'execution',
      ...
      options: [
        { value: 1, emoji: '❌', label: t('executionPoor'), tone: 'negative' },
        { value: 2, emoji: '🥣', label: t('executionOk'), tone: 'neutral' },
        { value: 3, emoji: '👨‍🍳', label: t('executionGreat'), tone: 'positive' },
      ],
    },
  ], [t]);
  ...
  return (
    ...
    >
      ...
      <p className="m-0 font-sans text-[11px] text-text-muted">
        {t('answeredCount', { count: data.answered })}
        {data.avg !== null && (
          <span
            dangerouslySetInnerHTML={{
              __html: t.raw('averageSuffix').replace('{avg}', data.avg.toFixed(1)),
            }}
          />
        )}
      </p>
