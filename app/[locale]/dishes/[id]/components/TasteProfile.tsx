...
export default function TasteProfile({ aggregates }: TasteProfileProps) {
  const t = useTranslations('dish.tasteProfile');
        ...
        <div>
          ...
          {hasTags ? (
            <ul className="mt-4 flex flex-col gap-2.5">
              {tagsTop.slice(0, 8).map((tag) => (
                <TasteBar key={tag.tag} label={tag.tag} count={tag.count} max={maxTagCount} />
              ))}
            </ul>
          ) : (
          ...
          <div>
            ...
            <ul className="mt-3 flex flex-wrap gap-1.5">
              {hasPros ? (
                ...
                <li className="font-sans text-xs text-text-muted">{t('noPros')}</li>
              )}
            </ul>
          </div>
          <div>
            ...
            <ul className="mt-3 flex flex-wrap gap-1.5">
              {hasCons ? (
                ...
                <li className="font-sans text-xs text-text-muted">{t('noCons')}</li>
              )}
            </ul>
          </div>
        </div>
