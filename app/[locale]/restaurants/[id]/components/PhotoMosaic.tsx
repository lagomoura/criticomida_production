...
export default function PhotoMosaic({ photos, totalCount, googlePhotos }: PhotoMosaicProps) {
  const t = useTranslations('restaurant.photoMosaic');
          ...
          <p className="mt-1 text-sm text-[var(--color-espresso-soft)]">
            {totalCount > 0 &&
              (totalCount === 1
                ...
                : t('communityMany', { count: totalCount }))}
            {totalCount > 0 && validGooglePhotos.length > 0 && ' · '}
            {validGooglePhotos.length > 0 && t('fromGoogle', { count: validGooglePhotos.length })}
          </p>
        </div>
      </header>
        ...
        {photos.map((p, i) => (
          <li key={p.id}>
            ...
            >
              <Image
                src={p.url}
                alt={p.alt_text ?? t('altByUser', { dish: p.dish_name, user: p.user_display_name })}
                width={600}
                height={800}
