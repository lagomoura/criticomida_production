...
export default function RestaurantHero({
  ...
  const t = useTranslations('restaurant.header');
      ...
      {hasCover && (
        <div className="relative mb-4 h-48 w-full overflow-hidden rounded-2xl bg-neutral-100 sm:h-64">
          <Image
            ...
            alt={t('photoAlt', { name: restaurant.name })}
            fill
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm text-neutral-600">
          ...
          <span className="text-neutral-400">
            {restaurant.review_count === 1
              ? t('reviewOne', { count: restaurant.review_count })
              : t('reviewMany', { count: restaurant.review_count })}
          </span>
          {dishCount > 0 && (
            <span className="text-neutral-400">
              · {dishCount === 1
                ? t('dishOne', { count: dishCount })
                : t('dishMany', { count: dishCount })}
            </span>
          )}
        </div>
