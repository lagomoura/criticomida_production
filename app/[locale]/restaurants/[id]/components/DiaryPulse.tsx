  ...
  return (
    <section className="rounded-3xl bg-[var(--color-carbon)] p-6 text-white sm:p-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Stat label={t('uniqueVisitors')} value={stats.unique_visitors} />
        <Stat label={t('totalVisits')} value={stats.visits_total} />
        <Stat label={t('thisWeek')} value={stats.visits_last_7d} highlight />
      </div>
      ...
      {stats.most_ordered_dish && (
        <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-white/10 pt-4 text-sm">
          ...
          <span className="text-white/60">
            {t('reviewsSuffix', { count: stats.most_ordered_dish.review_count })}
          </span>
        </div>
      )}
