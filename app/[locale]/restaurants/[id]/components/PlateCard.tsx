...
export default function PlateCard({
  ...
  const t = useTranslations('restaurant.plateCard');
  const tReview = useTranslations('restaurant.plateCard.review');
  ...
  const portionLabel = plate.portion || t('portionMedium');
  ...
  async function handleReviewSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        setReviewError(
          typeof err.detail === 'string' ? err.detail : tReview('submitFailed')
        );
      } else {
        setReviewError(tReview('submitFailed'));
      }
    } finally {
  ...
  return (
    <div
      ...
      aria-label={t('ariaLabel', { name: plate.name, date: formattedDate })}
      id={`plate-${idx}`}
    >
      <div
        className={
        ...
        >
          ...
          >
            <div className="flex flex-wrap items-center gap-3">
              ...
              {visitedWith && (
                <span className="flex items-center gap-1" title={t('companyTitle')}>
                  <span aria-hidden>👥</span>
            ...
            <div className="mt-3 border-t border-amber-300/30 pt-3">
              {reviewSuccess ? (
                <p className="text-sm font-medium text-green-700">
                  <span aria-hidden>✅</span> {tReview('successMessage')}
                </p>
              ) : reviewOpen ? (
                <form onSubmit={handleReviewSubmit} className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <textarea
                      ...
                      placeholder={tReview('notePlaceholder')}
                      disabled={reviewSubmitting}
                    />
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-neutral-600">{tReview('wouldOrderAgainPrompt')}</span>
                    <div className="form-check form-check-inline mb-0">
                      <input
                        ...
                        disabled={reviewSubmitting}
                      />
                      <label className="form-check-label text-xs" htmlFor={`woa-yes-${idx}`}>{tReview('yes')}</label>
                    </div>
                    <div className="form-check form-check-inline mb-0">
                      <input
                        className="form-check-input"
                        ...
                        disabled={reviewSubmitting}
                      />
                      <label className="form-check-label text-xs" htmlFor={`woa-no-${idx}`}>{tReview('no')}</label>
                    </div>
                  </div>
                  {reviewError && (
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={reviewSubmitting}
                    >
                      {reviewSubmitting ? tReview('submitting') : tReview('submit')}
                    </button>
                    <button
                      type="button"
