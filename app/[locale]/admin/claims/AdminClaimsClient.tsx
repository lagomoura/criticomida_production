...
const METHOD_KEY: Record<string, string> = {
  domain_email: 'methodDomain',
  google_business: 'methodGoogle',
  manual_admin: 'methodManual',
  phone_callback: 'methodPhone',
};
...
export default function AdminClaimsClient() {
  ...
  const TABS: { value: ClaimStatus; label: string }[] = useMemo(() => [
    { value: 'pending', label: t('tabPending') },
    { value: 'verified', label: t('tabVerified') },
    { value: 'rejected', label: t('tabRejected') },
    { value: 'revoked', label: t('tabRevoked') },
  ], [t]);
    ...
    <article className="flex flex-col gap-3 rounded-2xl border border-border-default bg-surface-card p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="font-sans text-xs uppercase tracking-wider text-text-muted">
            {METHOD_KEY[claim.verification_method]
              ? t(METHOD_KEY[claim.verification_method])
              : claim.verification_method}
            {' · '}
            <time dateTime={claim.submitted_at}>
              {formatRelativeTime(claim.submitted_at, locale)}
            </time>
          </p>
          <Link
      ...
      <div className="rounded-md bg-surface-subtle p-3 text-sm">
        <p className="font-sans text-text-secondary">
          <span className="font-medium">{t('claimantLabel')}</span>{' '}
