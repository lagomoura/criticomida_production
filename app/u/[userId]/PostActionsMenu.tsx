...
export default function PostActionsMenu({ post, onClose, onDelete, onEdit }: PostActionsMenuProps) {
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      setDeleting(false);
      setError('No se pudo eliminar la reseña. Probá de nuevo.');
    }
  }
  ...
  if (confirming) {
    return (
      <Modal
        ...
        description={`Vas a eliminar tu reseña de "${post.dish.name}". Esta acción no se puede deshacer.`}
        size="sm"
        busy={deleting}
        footer={
          <>
